...
from utils.cgl_negative_samples import compute_cgl_negative_samples
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        cgl_neg_samples = compute_cgl_negative_samples(img_tensor, labels_out)

        return torch.from_numpy(img), labels_out, self.img_files[index], shapes, cgl_neg_samples

    @staticmethod
    def collate_fn(batch):
        img, label, path, shapes, cgl_neg_samples = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, cgl_neg_samples
